...
public class VentanaJuego extends JPanel {
	...
	private HashMap<Integer, int[]> coordenadasNodo;


	public VentanaJuego(PantallaPrincipal pantallaPrincipal, WeightedGraph grafo) {
		this.pantallaPrincipal = pantallaPrincipal;
		this.grafo = grafo;
		this.coordenadasNodo = coordenadasNodo;
		iniciarJuego();
	}
	...
	public void paintComponent(Graphics g) {
	   ...
	   //TODO : Coordenadas para los nodos pero esta limitado a 5. Ver como hacer para poner los que sea
	    int[][] coordenadas = {
	        {100, 100}, {200, 200}, {300, 100}, {400, 200}, {500, 100}
	    };
	    ...
	    for (int i = 0; i < grafo.tamaño(); i++) {
	        for (int j = 0; j < grafo.tamaño(); j++) {
	            if (grafo.checkEdge(i, j)) {
	            	dibujo.drawLine(coordenadas[i][0] + 15, coordenadas[i][1] + 15, 
	                             coordenadas[j][0] + 15, coordenadas[j][1] + 15);
	                dibujo.drawString(String.format("%.2f", grafo.getEdgeWeight(i, j)),
	                               (coordenadas[i][0] + coordenadas[j][0]) / 2,
	                               (coordenadas[i][1] + coordenadas[j][1]) / 2); // Peso de la arista
	            }
	        }
	    }
	}
